...
A partir da pasta **`database/scripts/`** (com venv e dependências já instaladas):
...
From this folder (`database/scripts/`):
...
**Caminho:** o `cd` tem de apontar para a pasta `database/scripts/` dentro do clone do projeto (substitui `/caminho/para/` pelo caminho real; `…` não é válido no terminal).
...
Optional config: copy `.env.example` → `.env` in **`database/scripts/`** (not committed).
...
- **Default path:** `database/scripts/generated/utilizadores_examination.csv` (folder is gitignored).
...
2. `load_env_from_script_dir()` — optional `database/scripts/.env`.
...
- **Default:** `database/scripts/generated/tipos_examination.csv`
...
- `.env.example` — template for `database/scripts/.env`
