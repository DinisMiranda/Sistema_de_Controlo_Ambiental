...
function updateRoomData(roomId) {
  ...
  if (tempValue) {
    animateValue(tempValue, parseFloat(tempValue.textContent) || 0, roomData.temperature, 500);
  }
  
  // Atualizar Umidade
  const humidityValue = document.getElementById('humidity-value');
  if (humidityValue) {
    animateValue(humidityValue, parseFloat(humidityValue.textContent) || 0, roomData.humidity, 500);
  }
  ...
  updateComfortStatus(roomData.status, roomData.statusText, roomData.statusIcon);
...
function updateComfortStatus(status, text, icon) {
  ...
  statusIndicator.classList.remove('status-comfortable', 'status-attention', 'status-alert');
...
function animateValue(element, start, end, duration) {
  ...
  const timer = setInterval(() => {
    ...
    const displayValue = Number.isInteger(end) ? Math.round(current) : current.toFixed(1);
