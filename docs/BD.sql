...
CREATE TABLE IF NOT EXISTS `sistema_controlo_ambiental2`.`Casas` (
  ...
  PRIMARY KEY (`id_casa`),
  UNIQUE INDEX `email` (`morada` ASC) VISIBLE,
  INDEX `fk_Casas_sensores1_idx` (`sensores_id_sensor` ASC) VISIBLE,
  INDEX `fk_Casas_atuadores1_idx` (`atuadores_id_atuador` ASC) VISIBLE,
  CONSTRAINT `fk_Casas_sensores1`
    FOREIGN KEY (`sensores_id_sensor`)
    REFERENCES `sistema_controlo_ambiental2`.`sensores` (`id_sensor`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Casas_atuadores1`
...
CREATE TABLE IF NOT EXISTS `sistema_controlo_ambiental2`.`Utilizadores` (
  ...
  `Casas_id_casa` INT NOT NULL,
  PRIMARY KEY (`id_administrador`),
  UNIQUE INDEX `email` (`email` ASC) VISIBLE,
  INDEX `fk_Utilizadores_Casas1_idx` (`Casas_id_casa` ASC) VISIBLE,
  CONSTRAINT `fk_Utilizadores_Casas1`
    FOREIGN KEY (`Casas_id_casa`)
...
CREATE TABLE IF NOT EXISTS `sistema_controlo_ambiental2`.`parametros_automaticos` (
  ...
  INDEX `fk_parametros_automaticos_atuadores1_idx` (`atuadores_id_atuador` ASC) VISIBLE,
  CONSTRAINT `fk_parametros_automaticos_atuadores1`
    FOREIGN KEY (`atuadores_id_atuador`)
    REFERENCES `sistema_controlo_ambiental2`.`atuadores` (`id_atuador`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `sistema_controlo_ambiental2`.`registos_consumo` (
  ...
  INDEX `fk_registos_consumo_leituras_sensor1_idx` (`leituras_sensor_id_leitura` ASC) VISIBLE,
  CONSTRAINT `fk_registos_consumo_leituras_sensor1`
